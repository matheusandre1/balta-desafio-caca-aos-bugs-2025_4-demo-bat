...
public class OrderMethods
{
    ...
    public static async Task<OrderDto> BuildDtoAsync(Order order, IRepository<Product> productRepository, IMapper mapper)
    {
        ...
        foreach (var line in order.Lines)
        {
            ...
            var lineDto = mapper.Map<OrderLineDto>(line);

            linesDto.Add(new OrderLineDto(lineDto.Id, lineDto.OrderId, lineDto.Quantity, lineDto.Total, lineDto.ProductId, productDto));

        }

        var customerDto = mapper.Map<CustomerResponse>(order.Customer);

        return new OrderDto(order.Id, order.CustomerId, customerDto!, order.CreatedAt, order.UpdatedAt, linesDto);
    }
}
